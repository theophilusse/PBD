...
tasks.register("printRuntimeClasspath") {
    group = "pbd"
    description = "Prints the full runtime classpath (project classes + every " +
        "dependency jar - JOML, LWJGL, ...) between two markers, for the " +
        "Blender add-on's PbdConvertMain/PbdToTileGeometryMain subprocess " +
        "calls to parse out. Exists because those calls used to construct a " +
        "classpath by hand (just the build/classes directory, no dependency " +
        "jars at all) - worked fine for classes that happen to only touch " +
        "java.* internally, threw ClassNotFoundException/NoClassDefFoundError " +
        "on org.joml.Vector3fc the moment anything reachable from main() " +
        "touched a class from an actual dependency. Delegating to Gradle's own " +
        "dependency resolution here, the same way build/run/convertTileGeometry " +
        "already do via sourceSets.main.runtimeClasspath, is the fix - not " +
        "trying to hand-enumerate every jar Gradle might have decided to pull in."
    doLast {
        println("PBD_CLASSPATH_START" + sourceSets["main"].runtimeClasspath.asPath + "PBD_CLASSPATH_END")
    }
}
